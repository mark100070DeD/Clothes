...
db = storage.db_init()
total = storage.subscribers_count(db)
rows = storage.subscribers_recent(db, total or 1)
owner = config.load_chat_id()

print(f"База: {config.DB_PATH}")
print(f"Пользователей в базе: {total}")
print()
for user_id, ts, name, username in rows:
    ...
    tag = username or "без username"
    mine = "  <-- это ты" if user_id == owner else ""
    print(f"  {who}{mine}")
    print(f"      {tag}, id {user_id}, вошёл {when}")
...
print(f"Готовых карточек в витрине для /start: {storage.deals_count(db)}")
