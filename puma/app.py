"""Режимы запуска бота. Выбираются флагом в bot.py:
    (без флага)  живёт постоянно: слушает /start и проверяет сайт раз в INTERVAL_MIN
    --once       обход сайта + разбор команд, потом выход (часовой воркфлоу)
    --answer     только разбор команд, без обхода (быстрый воркфлоу)
"""
import asyncio
import logging
import sys
